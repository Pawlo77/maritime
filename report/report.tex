...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{images/graph_rotated.png}
...
This section details the implementation of data ingestion, storage, and analysis processes. Table~\ref{tab:process_schedule} summarizes the key processes, their data sources, tools used, and scheduling, providing an overview of the system's operational workflow. Details are provided in Sections~\nameref{sec:ais_stream_implementation}, \nameref{sec:marinesia_implementation}, \nameref{sec:open_meteo_implementation}, and \nameref{sec:analysis_implementation}. Analysis is limited to spark views only, without dashboards - they are not part of the project.

\begin{table}[H]
\centering
\caption{Schedule of Data Ingestion and Analysis Processes}
\label{tab:process_schedule}

\begin{tabularx}{\textwidth}{p{5cm} p{2.5cm} p{5cm} p{2cm}}
\toprule
\textbf{Process} & \textbf{Source / Method} & \textbf{Tool / DAG} & \textbf{Schedule} \\
\midrule
...
\subsection{AIS Stream API}
\label{sec:ais_stream_implementation}
...
\subsection{Marinesia API}
\label{sec:marinesia_implementation}
...
\subsection{Open-Meteo API}
\label{sec:open_meteo_implementation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.3\textwidth]{images/image copy 4.png}
...
\begin{figure}[H]
    ...
    \caption{Working docker containers.}
    \label{fig:hdfs_analysis}
\end{figure}
